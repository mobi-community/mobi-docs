...
- 쿼리 문자열
  - 실행 시 요청하는 데이터가 HTTP 요청 URL의 일부로 포함되어 보내집니다.
  - URL이 브라우저의 히스토리에 저장되고 로그에 기록되며 웹 서버에도 기록되므로
    보안에 민감한 정보를 다룰 때 문제가 될 수 있습니다.

🌟 POST
- 지정된 리소스에 엔티티를 제출합니다.
- 보통 서버의 상태 변화 또는 부수 효과를 일으킵니다.
- 요청하는 데이터를 HTTP 요청 본문에 포함시켜 보내며 이 데이터는 URL에 노출되지 않습니다.
  ➡️ 아예 뜯어볼 수 없는 것은 아니지만 POST가 GET보다 보안적인 측면에서 우월하다고 할 수 있습니다.

🤔 보안적으로 안전하지 않다는 것은 무엇을 의미할까요?

- HTTP 통신 자체는 평문 통신입니다.
  - 평문 통신: 암호화 되지 않은 데이터를 그대로 전송하는 통신 방식
- 이러한 HTTP 자체 특성 상 데이터가 노출될 위험이 있으므로
  중요한 정보를 전송할 때에는 반드시 암호화하여 전송해야 합니다.
```
...
function Submit() {
  ...
  // 특정 form에 대한 상태 정보를 반환하므로 form 요소의 자식으로 정의되어야 합니다.
